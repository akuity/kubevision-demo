...
akuity argocd cluster get-agent-manifests \
  --instance-name=kubevision-demo dev | kubectl apply -f -

echo "Deploying Akuity Agent manifests to stg cluster."
kubectl config use-context k3d-stg
akuity argocd cluster get-agent-manifests \
  --instance-name=kubevision-demo stg | kubectl apply -f -

echo "Deploying Akuity Agent manifests to prd cluster."
kubectl config use-context k3d-prd
akuity argocd cluster get-agent-manifests \
  --instance-name=kubevision-demo prd | kubectl apply -f -

domain=$(awk -F'[/:]' '{print $4}' <<< "$AKUITY_SERVER_URL")
argocd login \
  "$(akuity argocd instance get kubevision-demo -o json | jq -r '.id').cd.${domain}" \
  --username admin \
